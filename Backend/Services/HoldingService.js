...
class HoldingService {
    ...
    static async getHoldings(userId) {
        return await HoldingModel.find({ userId });
    }

    /**
     * Retrieves all intraday positions for a specific user.
     */
    static async getPositions(userId) {
        return await PositionModel.find({ userId });
    }

    /**
     * Seeds demo portfolio with ₹50,000 test margin and 12 equity holdings.
     */
    static async seedDemoData(userId) {
        ...
        logger.info("Demo portfolio seeded", { userId, holdingsCount: seededHoldings.length });
    ...
    /**
     * Resets user portfolio, positions, orders, and ledger to clean state.
     */
    static async resetPortfolio(userId) {
        await HoldingModel.deleteMany({ userId });
        await PositionModel.deleteMany({ userId });
        ...
        logger.info("Portfolio reset to clean state", { userId });
